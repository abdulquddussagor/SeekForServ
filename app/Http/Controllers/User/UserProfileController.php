...
class UserProfileController extends Controller
{
    ...
    {
        // Authenticate the user, then enable 2FA for them
        $user = Auth::user();
        ...
        // Redirect to a view or route with a success message
    ...
    {
        // Authenticate the user, then disable 2FA for them
        $user = Auth::user();

        // Clear the secret key in the user's record
    ...
    public function enableTwoFactorAuthentication(Request $request)
    {
        ...
        return view('user.enable_two_factor')->with([
            'user' => $user,
            'qrCodeUrl' => $qrCodeUrl,
        ]);
    }
    ...
    public function verifyTwoFactorAuthentication(Request $request)
    {
        ...
        // Verify the code using the user's secret key
        $google2fa = new Google2FA();
        ...
        if ($isValidCode) {
            ...
            return redirect()->route('user.dashboard')->with('success', 'Two-factor authentication enabled successfully.');
        }
    ...
    public function show_ticket($id){
        ...
        return view('user.show_ticket', compact('ticket','messages'));
    }
    ...
    public function send_ticket_message(Request $request){
        ...
        return view('user.ticket_message_list', compact('messages','ticket'));
    }
    ...
    public function get_invoice($id){
        ...
        return view('user.invoice',compact('order','currency_icon','booking_address','package_features','additional_services','client','provider','refundRequest','completeRequest'));
